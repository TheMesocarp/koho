                ...
                for k in uppers {
                    let noise =
                        Matrix::rand(stalk_dim_upper, stalk_dim, self.device.clone(), self.dtype)
                            ...
                            .scale(noise_limit)
                            .map_err(KohoError::Candle)?;
                    let fix = VarMatrix::new(id.clone().add(&noise).map_err(KohoError::Candle)?.tensor, self.device.clone(), self.dtype).map_err(KohoError::Candle)?;
                ...
                if let Some(r) = self.restrictions.get(&(k, sigma_idx, tau_dim, *i)) {
                    ...
                    if let Some(incidence_sign) = self.interlinked.get(&(k, sigma_idx, tau_dim, *i))
                    {
                        if *incidence_sign < 0 {
                            ...
                            term = Vector::new(new, self.device.clone(), self.dtype).map_err(KohoError::Candle)?;
                        }
                    }
...
mod adjoint_and_laplacian_tests {
    ...
    fn create_line_sheaf() -> Result<CellularSheaf, KohoError> {
        ...
        // Create 2 vertices with 1D data
    ...
    fn test_k_coboundary_simple() -> Result<(), KohoError> {
        ...
        // Create input: vertex values [1.0, 2.0]
        ...
        // Compute coboundary (should give edge values)
        ...
        let edge_val = edge_cochain[0]
            .inner()
            .squeeze(1).map_err(KohoError::Candle)?
            .squeeze(0).map_err(KohoError::Candle)?
            .to_scalar::<f32>()
            .map_err(KohoError::Candle)?;
        println!("here we are after edge val");
        println!("Coboundary result: {}", edge_val);
        assert!((edge_val - (-4.0)).abs() < 1e-6, 
    ...
    fn test_k_adjoint_coboundary_simple() -> Result<(), KohoError> {
        ...
        // Create input: edge value [5.0]
        let e0 = Vector::from_slice(&[5.0f32], 1, Device::Cpu, DType::F32)
            .map_err(KohoError::Candle)?;
        let edge_cochain = vec![e0];
        
        // Compute adjoint coboundary (should give vertex values)
        ...
        assert!((v0_val - 10.0).abs() < 1e-6, 
                "Expected v0=10.0, got {}", v0_val);
        assert!((v1_val - (-15.0)).abs() < 1e-6, 
    ...
    fn test_k_hodge_laplacian_up_only() -> Result<(), KohoError> {
        ...
        assert!((result_vec[0][0] - (-8.0)).abs() < 1e-6, 
                "Expected v0=-8.0, got {}", result_vec[0][0]);
        assert!((result_vec[0][1] - 12.0).abs() < 1e-6, 
                "Expected v1=12.0, got {}", result_vec[0][1]);
    ...
    fn test_hodge_laplacian_triangle() -> Result<(), KohoError> {
        ...
        for v in 0..3 {
            for e in 0..3 {
                // Check if vertex v is incident to edge e
        ...
        let result = sheaf.k_hodge_laplacian(0, input, false)?;
        println!("Triangle Laplacian result: {:?}", result.inner().to_vec2::<f32>());
    ...
    fn test_adjoint_with_zero_restriction() -> Result<(), KohoError> {
        ...
        // Override one restriction to be zero
        ...
        // v0 should be 0 due to zero restriction
        let v0_val = vertex_cochain[0]
            .inner()
            .squeeze(1).map_err(KohoError::Candle)?
            .squeeze(0).map_err(KohoError::Candle)?
...
mod generated_restrictions_tests {
    ...
    fn create_triangle_with_generated_restrictions() -> Result<CellularSheaf, KohoError> {
        ...
        // Use the generate_initial_restrictions method instead of manually setting them
        let noise_level = 0.1;
    ...
    fn test_triangle_with_generated_restrictions() -> Result<(), KohoError> {
        ...
        // Count expected restrictions
        // For 0->1: each vertex should have restrictions to its incident edges
        // For 1->2: each edge should have restrictions to the face
        ...
        // Verify that restrictions were created
        let total_expected = expected_0_to_1_restrictions + expected_1_to_2_restrictions;
        assert_eq!(sheaf.restrictions.len(), total_expected, 
                   "Should have the correct number of restriction maps");
        
        // Verify that restrictions have correct dimensions (1x1 in this case)
        for ((from_dim, from_idx, to_dim, to_idx), restriction) in &sheaf.restrictions {
            ...
            // Verify the restriction value is close to 1.0 (identity + small noise)
            let value = restriction.inner().as_tensor()
                .squeeze(0).map_err(KohoError::Candle)?
                .squeeze(0).map_err(KohoError::Candle)?
                .to_scalar::<f32>().map_err(KohoError::Candle)?;
            ...
            assert!((value - 1.0).abs() <= 0.5, 
                    "Restriction value {} should be close to 1.0 (identity + noise)", value);
        }
    ...
    fn test_generated_restrictions_coboundary_computation() -> Result<(), KohoError> {
        ...
        println!("Vertex values: {:?}", vertex_cochain.inner().to_vec2::<f32>());
    ...
    fn test_different_noise_levels() -> Result<(), KohoError> {
        ...
        for &noise_level in &[0.0f32, 0.05f32, 0.1f32, 0.5f32] {
            ...
            for (_, restriction) in &sheaf.restrictions {
                let value = restriction.inner().as_tensor()
                    .squeeze(0).map_err(KohoError::Candle)?
                    .squeeze(0).map_err(KohoError::Candle)?
                    .to_scalar::<f32>().map_err(KohoError::Candle)?;
                ...
                assert!(deviation <= noise_level * 3.0f32, // Allow for some randomness 
