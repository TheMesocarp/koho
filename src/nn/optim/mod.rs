...
pub fn create_optimizer<'a>(
    ...
    match kind {
        OptimKind::SGD => {
            if let OptimizerParams::SGD { momentum, weight_decay, .. } = params {
                let mut opt = SGD::new(lr)?;
                if let Some(m) = momentum {
                    if let Momentum::Nesterov(val) = m {
                        opt = opt.momentum(val);
                    }
                }
                if let Some(wd) = weight_decay {
                    if let Decay::WeightDecay(val) = wd {
        ...
        OptimKind::Adam | OptimKind::AdamW => {
            if let OptimizerParams::AdamW { beta_1, beta_2, eps, weight_decay, .. } = params {
                let mut opt = Adam::new(vars, lr)?
                    .betas(beta_1, beta_2)
                    .eps(eps);
        ...
        OptimKind::RMSprop => {
            Ok(Box::new(RMSprop::new(vars, lr)?))
        }
    }
}