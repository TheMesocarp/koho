        ...
        for (i, var) in vars.into_iter().enumerate() {
            if let Some(grad) = grads.get(var) {
                ...
                let denom = if self.centered {
                    // Centered RMSprop
                    let avg_t = self.avg[i].as_ref().ok_or(Error::Msg("failed to initialize centering for RMSprop".to_string()))?;
                    let avg_new = ((avg_t * self.alpha)? + (effective_grad.clone() * (1.0 - self.alpha))?)?;
